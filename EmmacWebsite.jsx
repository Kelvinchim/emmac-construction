export default function EmmacWebsite() {
  ...
  const projects = [
    ...
    { title: "River Training & Dykes", category: "Civil Works", client: "Roads Authority", period: "2018–2021", hero: BASE + "Project-irrigation 9.jpg" },
    ...
    <div className="min-h-screen bg-neutral-950 text-white font-sans">
      <header className="sticky top-0 z-50 border-b border-white/10 bg-neutral-950/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-10">
          <div>
            <div className="text-2xl font-black tracking-[0.25em]" style={{color:'#c2522a'}}>EMMAC</div>
          ...
          <a href="#contact" className="rounded-full px-5 py-2 text-sm font-semibold text-white transition hover:opacity-90" style={{background:'#c2522a'}}>Get in Touch</a>
        </div>
      </header>
      <section className="relative overflow-hidden border-b border-white/10 min-h-[90vh] flex items-center">
        <div className="absolute inset-0 grid grid-cols-2 grid-rows-2 opacity-20">
          ...
          <img src={BASE + "Project-irrigation 9.jpg"} className="w-full h-full object-cover" alt="" />
        </div>
        <div className="absolute inset-0 bg-gradient-to-r from-neutral-950 via-neutral-950/85 to-neutral-950/40" />
        <div className="absolute left-0 top-0 bottom-0 w-1" style={{background:'#c2522a'}} />
        <div className="relative z-10 mx-auto grid max-w-7xl gap-12 px-6 py-24 lg:grid-cols-[1.2fr_0.8fr] lg:px-10">
          <div className="pl-6">
            <div className="mb-6 inline-flex rounded-full border px-4 py-2 text-xs uppercase tracking-[0.3em]" style={{borderColor:'rgba(194,82,42,0.4)',color:'rgba(194,82,42,0.9)'}}>Civil & Building Contractors · Malawi</div>
            ...
            <p className="mt-6 max-w-xl text-base leading-7 text-white/55 md:text-lg">EMMAC Construction delivers infrastructure, irrigation, and building projects across Malawi — on time, to spec, with accountability.</p>
            <div className="mt-8 flex flex-wrap gap-4">
              <a href="#projects" className="rounded-full px-6 py-3 text-sm font-bold text-white transition hover:opacity-90" style={{background:'#c2522a'}}>View Projects</a>
              <a href="#contact" className="rounded-full border border-white/20 px-6 py-3 text-sm font-semibold text-white transition hover:bg-white/5">Get in Touch</a>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            {stats.map((item) => (
              <div key={item.label} className="rounded-2xl border border-white/10 bg-white/[0.04] p-6 backdrop-blur" style={{borderLeft:'3px solid #c2522a'}}>
                <div className="text-4xl font-black tracking-tight">{item.value}</div>
                <div className="mt-2 text-xs uppercase tracking-[0.2em] text-white/45">{item.label}</div>
              </div>
            ))}
          </div>
        </div>
      </section>
      <section id="services" className="border-b border-white/10 bg-neutral-900/40">
        <div className="mx-auto max-w-7xl px-6 py-20 lg:px-10">
          <p className="text-xs font-semibold uppercase tracking-[0.3em]" style={{color:'#c2522a'}}>What We Do</p>
          <h2 className="mt-3 text-3xl font-bold tracking-tight md:text-4xl">Core Services</h2>
          <div className="mt-10 grid gap-6 md:grid-cols-3">
            {services.map((s) => (
              <div key={s.title} className="rounded-2xl border border-white/10 bg-neutral-900 p-7" style={{borderTop:'2px solid #c2522a'}}>
                <h3 className="text-xl font-semibold">{s.title}</h3>
                <p className="mt-4 text-sm leading-7 text-white/55">{s.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="projects" className="border-b border-white/10">
        <div className="mx-auto max-w-7xl px-6 py-20 lg:px-10">
          <p className="text-xs font-semibold uppercase tracking-[0.3em]" style={{color:'#c2522a'}}>Portfolio</p>
          <h2 className="mt-3 text-3xl font-bold tracking-tight md:text-4xl">Selected Projects</h2>
          <div className="mt-10 grid gap-6 lg:grid-cols-2">
            {projects.map((project) => (
              <div key={project.title} className="group overflow-hidden rounded-2xl border border-white/10 bg-neutral-900">
                <div className="relative h-64 overflow-hidden">
                  ...
                  <div className="absolute bottom-4 left-6">
                    <span className="rounded-full px-3 py-1 text-xs uppercase tracking-[0.2em] text-white font-semibold" style={{background:'rgba(194,82,42,0.9)'}}>
                      {project.category}
                    </span>
                  </div>
                </div>
                <div className="p-6">
                  ...
                  <div className="mt-3 flex gap-6 text-sm text-white/45">
      ...
      <section id="clients" className="border-b border-white/10 bg-neutral-900/40">
        <div className="mx-auto max-w-7xl px-6 py-20 lg:px-10">
          <p className="text-xs font-semibold uppercase tracking-[0.3em]" style={{color:'#c2522a'}}>Who We Work With</p>
          ...
          <div className="mt-10 grid grid-cols-2 gap-4 md:grid-cols-3">
            {clients.map((c) => (
              <div key={c.name} className="rounded-2xl border border-white/10 bg-neutral-900 px-6 py-5" style={{borderLeft:'3px solid #c2522a'}}>
      ...
      <section id="contact">
        <div className="mx-auto grid max-w-7xl gap-10 px-6 py-24 lg:grid-cols-2 lg:px-10">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.3em]" style={{color:'#c2522a'}}>Contact</p>
            <h2 className="mt-3 text-3xl font-bold tracking-tight md:text-4xl">Start a conversation.</h2>
            <p className="mt-4 max-w-sm text-white/50 leading-7">Whether it is a large infrastructure project or a commercial build, we are ready to deliver.</p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-neutral-900 p-8" style={{borderTop:'2px solid #c2522a'}}>
            ...
            <a href="mailto:info@emmacmw.net" className="mt-8 block w-full rounded-full px-6 py-3 text-center text-sm font-bold text-white transition hover:opacity-90" style={{background:'#c2522a'}}>Send an Email</a>
          </div>
        </div>
      </section>
      <footer className="border-t border-white/10">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-6 lg:px-10">
          <div>
            <div className="text-sm font-black tracking-[0.25em]" style={{color:'#c2522a'}}>EMMAC</div>
